...
parser.add_argument('--test_dir', type=str, default='./tools/methods/SCI3D-main/ExtremeCacti_in', help='path to test set')
parser.add_argument('--model_dir', type=str, default='./tools/methods/SCI3D-main/checkpoint', help='trained or pre-trained model directory')
parser.add_argument('--result', type=str, default='./tools/methods/SCI3D-main/ExtremeCacti_out', help='results for reconstructed frames')
...
parser.add_argument('--temporal_patch', type=int, default=8,
...
def test_simulation():
    ...
    for i in range(len(test_list)):
        ...
        meas = Training_data['meas']
        meas = torch.from_numpy(meas).type(torch.FloatTensor).to(device)
        ...
        with torch.no_grad():
            out_test = None
            meas_test = meas.unsqueeze(0).unsqueeze(0).permute(4, 0, 1, 2, 3)
            ...
            start = time.time()
            for k in range(np.size(meas_test,0)):
                aux = model(meas_test[k].unsqueeze(0), mask)
                if out_test is None:
                    out_test = aux
                else:
                    out_test = torch.concat((out_test,aux),0)
            torch.cuda.synchronize()
            end = time.time()
            ...
            sio.savemat(os.path.join(args.result, test_list[i]), {'pic': out_test.cpu().numpy()})
