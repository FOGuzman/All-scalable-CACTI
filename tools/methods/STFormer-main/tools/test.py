...
def parse_args():
    ...
    parser.add_argument("--mask_path",type=str,default='tools/methods/test_datasets/mask/mask.mat')
...
def main():
    ...
    cfg.test_data.mask_path = args.mask_path

    device = args.device
    config_name = osp.splitext(osp.basename(args.config))[0]
    if args.work_dir is None:
        args.work_dir = osp.join('./work_dirs',config_name)
    mask,mask_s = generate_masks(args.mask_path,cfg.test_data.mask_shape)
    cr = mask.shape[0]
    ...
    logger.info('GPU info:\n' 
            ...
            env_info + '\n' +
            dash_line) 
    cfg.test_data.data_root = "./tools/methods/STFormer-main/test_datasets/simulation"
    print(cfg.test_data.data_root)
    ...
    for data_iter,data in enumerate(data_loader):
        ...
        sio.savemat(image_dir, {'recon': out})
    if time_count==0:
        time_count=1
