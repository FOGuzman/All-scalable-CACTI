...
data_path = "./tools/methods/RevSCI-net-master/train"
test_path1 = "./tools/methods/RevSCI-net-master/ExtremeCacti_in/"
...
def test(test_path, epoch, result_path, model, args):
    ...
    for i in range(len(test_list)):
        ...
        meas_pic = scio.loadmat(test_path + '/' + test_list[i])  
        meas = np.array(meas_pic['meas']).astype(float)
        meas = torch.from_numpy(meas).cuda().float()
        if meas.ndim<3:
           meas = torch.unsqueeze(meas,-1)
        meas = torch.permute(meas,(2,0,1))
        ...
        with torch.no_grad():
            ...
            a = test_list[i]
            name1 = result_path + '/data_out.mat'
...
if __name__ == '__main__':
    ...
    result_path = './tools/methods/RevSCI-net-master/' + 'ExtremeCacti_out' + '/'
    ...
    if args.last_train != 0:
        rev_net = torch.load(
            './tools/methods/RevSCI-net-master/model/' + args.model_save_filename + "/RevSCInet_model_epoch_{}.pth".format(args.last_train),map_location='cuda:0')
